...
// 题3: BitToggle — 翻转 value 的第 bit 位，其余位不变。
...
{
    assert(bit < 32);
    return (value & (1U << bit)) == value;  // ← 这里有 bug
}
...
/* 题11: 不需要写代码。所有函数 PASS 后运行 make，观察位操作实验台输出。 */
